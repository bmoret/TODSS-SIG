create table attendance (id uuid not null, is_speaker boolean not null, state int4, person_id uuid not null, session_id uuid not null, primary key (id));
...
create table session (id uuid not null, description varchar(255), end_date timestamp, start_date timestamp, subject varchar(255), state varchar(255), contact_person_id uuid, sig_id uuid, primary key (id));
...
create table users (id uuid not null, password varchar(255), role varchar(255), username varchar(255) not null, person_id uuid, primary key (id));
...
alter table session add constraint FKilk650rpveid9iwuvov47kvpw foreign key (contact_person_id) references person;
...
alter table users add constraint FKd21kkcigxa21xuby5i3va9ncs foreign key (person_id) references person;
